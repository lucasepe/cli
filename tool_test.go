...
func TestToolNew(t *testing.T) {
	c := New("binaryName", "binaryDescription")

	// the tool should have binaryName and binaryDescription set
	if c.binaryName != "binaryName" {
		t.Error("tool binaryName is incorrect")
	}
	if c.description != "binaryDescription" {
		t.Error("tool description is incorrect")
	}
